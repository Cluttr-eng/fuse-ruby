  ...
  class FinQLFeatures
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `FuseClient::FinQLFeatures` initialize method"
      end

      # check to see if the attribute exists and convert string to symbol for hash key
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!self.class.attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `FuseClient::FinQLFeatures`. Please check the name to make sure it's valid. List of attributes: " + self.class.attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
      ...
      if attributes.key?(:'time_based')
        if (value = attributes[:'time_based']).is_a?(Array)
          self.time_based = value
        end
      end
      ...
      if attributes.key?(:'top_merchants')
        if (value = attributes[:'top_merchants']).is_a?(Array)
          self.top_merchants = value
        end
      end
      ...
      if attributes.key?(:'inferred')
        self.inferred = attributes[:'inferred']
      end
    end
    ...
    def hash
      [text, individual_merchant, time_based, interest, frequency, trend, top_merchants, comparison, merchant_categories, inferred].hash
    end
